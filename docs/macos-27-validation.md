...
Built an optimized arm64 0.1.6 (build 16) local test DMG with Apple Development signing. Verified its checksum, mounted it read-only, verified the enclosed app's strict deep signature and helper identifier/team requirement, checked its version and executable architecture, and detached it. This artifact is not notarized and is not an Intel build. The Mac became locked before the final installed release-build UI restart check; that check remains pending. Earlier installed debug-build TUN checks do not substitute for that final release UI check.
